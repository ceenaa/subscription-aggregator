...
The worker loads the same panel config used by `/inbounds`, calls each panel's `inbounds/list` endpoint, finds the configured `FIRST_PANEL_INBOUND_ID` and `SECOND_PANEL_INBOUND_ID`, and matches clients by `subId`. If a client exists on only one panel, it is skipped.

The worker only evaluates matched clients that are active on at least one panel. If the client is already disabled on both panels, it is skipped.

A matched client is disabled on both panels when any quota check is true:

- first panel total is nonzero and `allTime >= total`
- second panel total is nonzero and `allTime >= total`
- both totals are nonzero and normalized combined usage is at or over the lower quota

For the combined check, the worker scales the higher-quota panel down into the lower-quota panel's units:

```text
scale = higherTotal / lowerTotal
normalizedUsed = lowerUsed + higherUsed / scale
...
The page creates the same client on both configured panels. `FIRST_PANEL_PROXY=xray` sends the first `addClient` request through the Xray outbound. `SECOND_PANEL_PROXY=direct` sends the second request directly.
...
With those values, entering `5` in `Total Flow` sends `5 GiB` to the first panel and `2.5 GiB` to the second panel. `0` still means unlimited for every panel.
